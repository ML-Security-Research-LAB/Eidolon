...
class ConfigParser:
    ...
    def __init__(self, config_file):
        ...
        self.root_dir = self.config_dir.parents[1]
    ...
    def load_tokenizer(self):
        # Use the correct model path, subfolder, and revision for Stable Diffusion
        pretrained_model_name_or_path = self._config['tokenizer']  # This will still be 'CompVis/stable-diffusion-v1-4'
        subfolder = 'tokenizer'  # Explicitly point to the tokenizer subfolder
        revision = None  # Use the default revision unless specified otherwise
    ...
    def load_text_encoder(self):
        pretrained_model_name_or_path = self._config['text_encoder']
        subfolder = 'text_encoder'  # Specify the subfolder for the text encoder
        revision = None  # You can specify a revision if needed, or leave it as None
    ...
    def load_datasets(self):
        ...
        if 'txt' in dataset_name:
            ...
            if not dataset_path.is_absolute():
                dataset_path = self.root_dir / dataset_path
    ...
    def resolve_path(self, path):
        ...
        if resolved.is_absolute():
            return str(resolved)
        return str(self.root_dir / resolved)
    ...
    # New properties for replaced_character, trigger, and target_prompt
    @property
    def replaced_character(self):
        return self.injection.get('replaced_character')

    @property
    def trigger(self):
        return self.injection.get('trigger')

    @property
    def target_prompt(self):
        return self.injection.get('target_prompt')
    ...
    @property
    def base_model(self):
        return self._config.get('base_model', 'CompVis/stable-diffusion-v1-4')
